...
import {
  ...
  PhonePlanStep,
  PhonePlanDrillItem,
} from '../../../data/courses/english/phonePlansLesson';
import {
  translationService,
  TranslationLang,
...
interface PhonePlansLessonEngineProps {
...
export function PhonePlansLessonEngine({
  lesson,
  onExit,
  onLessonComplete,
}: PhonePlansLessonEngineProps) {
  ...
  const currentStep: PhonePlanStep = PHONE_PLANS_STEPS[currentStepIndex] || PHONE_PLANS_STEPS[0];
  const totalSteps = PHONE_PLANS_STEPS.length; // 22 steps + 23rd completion screen
  const currentDrill: PhonePlanDrillItem = currentStep?.drills[currentDrillIndex] || currentStep?.drills[0];
  ...
  const handleTypingComplete = useCallback(
    (stats: TypingSessionStats) => {
      ...
      // Record any mistakes in session list and reviewService
      if (stats.mistakes > 0 && currentDrill?.targetText) {
        setSessionMistakeWords((prev) =>
          prev.includes(currentDrill.targetText) ? prev : [...prev, currentDrill.targetText]
        );
  ...
  const avgAccuracy =
    accuracySamples.length > 0
      ...
      : Math.max(82, 100 - accumulatedMistakes * 2);
  ...
  useEffect(() => {
    if (isLessonCompleted && !hasRecordedRef.current) {
      ...
      progressService.recordExerciseCompletion({
        subjectId: 'english',
        exerciseId: 'en-phone-plans',
        exerciseTitle: 'Phone Plans',
        wpm: avgWpm,
    ...
    >
      ...
      {!isLessonCompleted && (
        <div className="w-full max-w-3xl mx-auto px-4 pt-3 flex flex-col gap-2">
          <div className="flex items-center justify-between text-xs font-mono">
            ...
            <div className="flex items-center gap-2">
              <span className="text-amber-400 font-semibold uppercase tracking-wider">
                Step {currentStep.stepNumber} of 22
              </span>
            ...
            <div className="flex items-center gap-1.5 bg-neutral-900 border border-neutral-800 rounded-lg px-2.5 py-1 text-xs">
              ...
              <select
                id="phone-plans-translation-select"
              ...
              >
                ...
                <option value="sv" className="bg-neutral-900 text-neutral-200">
                  Swedish
                </option>
          ...
          <div className="w-full h-1.5 bg-neutral-900 rounded-full overflow-hidden flex gap-0.5">
            {PHONE_PLANS_STEPS.map((step, idx) => (
              <div
                key={step.stepNumber}
        ...
        <AnimatePresence mode="wait">
          {!isLessonCompleted ? (
            ...
            >
              ...
              {/* Core Typing Interaction (useTypingEngine & TypingText) */}
              <div
              ...
              {/* Post Explanation Note (e.g. Step 3) */}
            ...
            >
              ...
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
                  PHONE PLANS COMPLETE ✓
                </h2>
                <p className="text-sm text-neutral-400 mt-1">
                  You have mastered essential vocabulary and conversational phrases by typing.
                </p>
              </div>
      ...
      <footer className="w-full py-2 px-3 sm:px-4 border-t border-neutral-900/60 flex items-center justify-between text-[11px] text-neutral-600 font-mono select-none">
        ...
        <div className="mx-auto sm:mx-0 text-center truncate">
          MY LEARNING · English · I learn by typing
        </div>
        <div className="hidden sm:block">
          Translation: {translationLang === 'so' ? 'Somali' : translationLang === 'sv' ? 'Swedish' : 'Off'}
        </div>
      </footer>
    </div>
  );
